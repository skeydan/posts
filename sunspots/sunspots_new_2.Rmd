...
These time series do have something in common with both image data and language data: Like language, they form sequences, and like images, they are made up of homogeneous components - at least if we stay with univariate time series, as we are going to do in this post.
...
_Note: When running the code, please execute the subsequent snippet before this one as it introduces a few needed variables (`FLAGS$batch_size`, `FLAGS$n_timesteps` and `n_predictions`). These are basically just the batch size used, the state length and the number of predictions, and are defined below.
...
This topic too, we'll leave to a separate discussion another time, but interestingly regularization using different settings of `dropout` and `recurrent_dropout` did not yield better generalization performance. This is probably related to characteristics of this special time series which we'll shortly comment on below.
...
![](images/pred_test.png)


That's not as good as on the training set, but not bad either! This time series is all but regular.
On the one hand, short term variability (over months) is really high. On the other, there seem to be important changes in amplitude in the long run. Those are necessarily hard to anticipate for any forecasting algorithm.

At this point, it would be easy to digress into a discussion of what are especially appropriate data to predict with LSTM, including comparison with traditional methods used in forecasting. We'll leave that to another post and instead, revert to our overall re-sampling setup for validation. 
...
![](images/backtested_test.png)



This has been a long post, and necessarily will have left a lot of questions open.

Stay tuned for upcoming posts more narrowly focused on specifics of tuning deep learning models, as well as - of course - further case studies exploring applications of deep learning to a wide range of real-world tasks!
